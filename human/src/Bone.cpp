...
void Bone::rescale(const vec3& v, bool child)
{
    if (child == false)
    {
        ...
        const vec3 d = (v - this->model->getScaling()) * 0.5;
        const vec3 n = d.multiply(mtls::normalize(this->model->getJoint() - this->model->getPosition()));
    ...
    else
    {
        ...
        const vec3 n = v.multiply(mtls::normalize(this->model->getJoint() - this->model->getPosition()));
        this->model->setPosition(this->model->getPosition() - n);
    }
}
